...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    public List<DonorDetailsActivity> getDonorList() {
        DonorDetailsActivity donorDetails = null;
        List<DonorDetailsActivity> donorD = new ArrayList<>();
        sqLiteDatabase = this.getReadableDatabase();

        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM Donors_Info", null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            donorDetails = new DonorDetailsActivity(cursor.getInt(0),cursor.getString(1),cursor.getString(2),cursor.getString(3),cursor.getString(4)
    ...
    public List<DonorDetailsActivity> getSearchedDonorList(String group, String location) {
        DonorDetailsActivity donorDetails = null;
        List<DonorDetailsActivity> donorD = new ArrayList<>();
        sqLiteDatabase = this.getReadableDatabase();
